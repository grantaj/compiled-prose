...
class BuildDirectoryIsolationTests(unittest.TestCase):
    ...
    def test_default_and_overridden_paths_cover_all_generated_outputs(self):
        ...
        default = self.run_make("-n", "-B", "IN=outline.md", "realise", env=default_env)
        self.assertEqual(default.returncode, 0, default.stderr)
        self.assertIn('"build/realise.tex"', default.stdout)
        self.assertIn('"build/errors/realise.md"', default.stdout)
    ...
    def test_environment_can_override_default_build_directory(self):
        ...
        result = self.run_make("-n", "-B", "IN=outline.md", "realise", env=env)
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn('"build-env/realise.tex"', result.stdout)
        self.assertIn('"build-env/errors/realise.md"', result.stdout)
    ...
    def test_two_builds_coexist_and_failure_diagnostics_are_isolated(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            first = self.run_make(
                f"BUILD_DIR={build_a}", f"IN={source}", f"LLM_RUNNER={runner}", "realise", env=env
            )
            self.assertEqual(first.returncode, 0, first.stderr)

            env["FAKE_OUTPUT_FILE"] = str(second_output)
            second = self.run_make(
                f"BUILD_DIR={build_b}", f"IN={source}", f"LLM_RUNNER={runner}", "realise", env=env
            )
            self.assertEqual(second.returncode, 0, second.stderr)
            self.assertEqual((build_a / "realise.tex").read_text(encoding="utf-8"), VALID_TEX_A)
            self.assertEqual((build_b / "realise.tex").read_text(encoding="utf-8"), VALID_TEX_B)

            env["FAKE_OUTPUT_FILE"] = str(failure_output)
            failed = self.run_make(
                "-B", f"BUILD_DIR={build_b}", f"IN={source}", f"LLM_RUNNER={runner}", "realise", env=env
            )
            self.assertNotEqual(failed.returncode, 0)
            self.assertEqual((build_a / "realise.tex").read_text(encoding="utf-8"), VALID_TEX_A)
            self.assertFalse((build_b / "realise.tex").exists())
            self.assertIn(
                "Missing source detail",
                (build_b / "errors" / "realise.md").read_text(encoding="utf-8"),
            )
    ...
    def test_clean_and_clobber_only_touch_selected_directory(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for build in (build_a, build_b):
                ...
                (build / "errors" / "realise.md").write_text("diagnostic", encoding="utf-8")
            ...
            self.assertTrue((build_b / "realise.tex").exists())
            self.assertTrue((build_b / "errors" / "realise.md").exists())
    ...
    def test_gitignore_uses_narrow_root_build_convention(self):
        ignored = ("build/realise.tex", "build-academic/final.tex", "build-test-a/errors/realise.md")
