...
Self-example release has two deliberately separate workflows:
...
2. **Publish self-example** is strictly keyless. The operator selects one target; the workflow finds that target's most recent successful retained compilation, automatically preserves unselected targets from the latest successful publication, verifies source compatibility, assembles the multi-target site, and deploys it. It never recompiles an existing artifact.
...
The normal operator workflow is deliberately simple: make explicitly authorised paid compile runs for a target until the latest successful candidate is the one you want, then run **Publish self-example** and choose that target. Run IDs remain internal provenance; they are not publication inputs. On later publications, unselected target renderings are preserved automatically from the most recent successful retained showcase, while the selected target is replaced by its newest successful retained compile candidate.

The publish assembler compares the exact authoritative outline bytes across the selected candidate and preserved showcase **and against the current repository `outline.md`**. Compiler repository commits may differ: that is intentional so an already-accepted rendering can be reused after workflow or publication code changes, provided the authored outline itself is unchanged. Each target keeps its own compilation commit/model/run provenance in the published site. If the authoritative outline changes, incompatible retained renderings fail closed rather than being mixed silently.

The academic-journal rendering also preserves the former top-level evidence paths (`peer-review.html`, `acceptance.html`, `artifacts/final.pdf`, and `artifacts/final.tex`) for link compatibility. The root `index.html` becomes the target showcase; the compatibility paths do not make the academic target canonical. Because those compatibility URLs require an academic rendering, a repository with no retained prior showcase must publish `journal_academic` first; this bootstrap constraint does not expose any run-ID plumbing.
