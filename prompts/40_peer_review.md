...
- The selected target defines acceptable realisation, including audience, style, explanatory explicitness, level of rigour, evidence visibility, attribution, citation presentation, structure, and coverage. Exhaustive conceptual coverage is the default within the core publication pipeline unless the selected target explicitly authorises summarisation, compression, selective omission, or presentation reordering.
...
2. Target realisation assurance: assess whether the revised artefact faithfully realises that source at the coverage, style, explanatory depth, rigour, structure, and evidence/attribution/citation presentation expected by the selected target. Do not import academic or otherwise stricter venue conventions that the selected target does not require. Do not demand exhaustive coverage when the target explicitly authorises reduced coverage, and do not demand formal citation apparatus when the target explicitly requires another presentation or none.

Classify every finding by where the defect belongs:
- SOURCE: fixing it requires an authorial choice or source change, including failure of the target-independent source-assurance floor (for example a missing necessary warrant, contradiction, material factual defect where determinable, missing conceptual scope boundary, or evidentiary/attribution support required by the claim's nature), additional source material needed to meet an explicit target-level rigour, evidence, attribution, or citation requirement, or material expansion of the argument.
